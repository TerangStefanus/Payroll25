...
namespace Payroll25.Controllers
{
    public class HonorPendadaranController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                var viewModel = new HonorPendadaranViewModel
                {
                    HonorPendadaranList = await DAO.ShowHonorPendadaranAsync(),
                    HonorPendadaran = new HonorPendadaranModel()
                };
        ...
        // GET: HonorPendadaranController/Details/5
        ...
        public async Task<IActionResult> Create(HonorPendadaranViewModel viewModel)
        {
            if (viewModel == null)
            {
                viewModel = new HonorPendadaranViewModel();
            }
            else if (viewModel != null)
            {
                var errors = new List<string>();

                if (viewModel.HonorPendadaran.ID_KOMPONEN_GAJI == 0)
                {
                    errors.Add("ID Komponen Gaji harus diisi.");

                }

                if (viewModel.HonorPendadaran.ID_BULAN_GAJI == 0)
                {
                    errors.Add("ID BULAN GAJI harus diisi.");

                }

                if (string.IsNullOrEmpty(viewModel.HonorPendadaran.NPP))
                {
                    errors.Add("NPP harus diisi.");

                }

                if (viewModel.HonorPendadaran.JUMLAH == 0)
                {
            ...
            viewModel.HonorPendadaranList = await DAO.ShowHonorPendadaranAsync();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult EditDetails(HonorPendadaranViewModel viewModel, int ID_Vakasi)
        {
            try
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(HonorPendadaranViewModel viewModel, int ID_Vakasi)
        {
            try
            {
