...
namespace Payroll25.DAO
{
    public class PayslipDosenDAO
    {
        ...
        public async Task<bool> CheckDataGajiDosen(int idBulanGaji, string npp)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
                {
                    string query = "SELECT COUNT(*) FROM [PAYROLL].[payroll].[TBL_PENGGAJIAN] JOIN [PAYROLL].[simka].[MST_KARYAWAN] ON TBL_PENGGAJIAN.NPP = MST_KARYAWAN.NPP WHERE [simka].[MST_KARYAWAN].STATUS_KEPEGAWAIAN = 'Kontrak' AND ID_BULAN_GAJI = @idBulanGaji AND TBL_PENGGAJIAN.NPP = @npp";
        ...
        public async Task<IEnumerable<HeaderPenggajianUserDosen>> GetHeaderPenggajianUserDosen(int idBulanGaji , string npp)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                string query = @"SELECT
                ...
                var headers = await conn.QueryAsync<HeaderPenggajianUserDosen>(query, new { idBulanGaji ,npp });
                return headers;
            }
        }
        ...
        public async Task<IEnumerable<DetailPenggajianUserDosenModel>> GetBodyPenggajianUserDosen(int idPenggajian)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                string query = @"SELECT 
                ...
                var result = await conn.QueryAsync<DetailPenggajianUserDosenModel>(query, new { idPenggajian });
                return result;
            }
        }
