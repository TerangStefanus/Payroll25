...
namespace Payroll25.DAO
{
    ...
    {
        public async Task<IEnumerable<HonorPascaSarjanaModel>> ShowHonorPascaSarjanaAsync()
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    ...
                    var query = @"SELECT
                                ...
                                WHERE 
                                MST_KOMPONEN_GAJI.ID_KOMPONEN_GAJI = 82";

                    var data = conn.Query<HonorPascaSarjanaModel>(query, parameters).ToList();
        ...
        public bool InsertVakasi(HonorPascaSarjanaModel.HonorPascaSarjanaViewModel viewModel)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    ...
                    var parameters = new
                    {
                        ID_KOMPONEN_GAJI = viewModel.HonorPascaSarjana.ID_KOMPONEN_GAJI,
                        ID_BULAN_GAJI = viewModel.HonorPascaSarjana.ID_BULAN_GAJI,
                        NPP = viewModel.HonorPascaSarjana.NPP,
                        JUMLAH = viewModel.HonorPascaSarjana.JUMLAH,
                        DATE_INSERTED = DateTime.Now,
                        DESKRIPSI = viewModel.HonorPascaSarjana.DESKRIPSI
                    };
        ...
        public bool UpdateVakasiHonor(HonorPascaSarjanaModel.HonorPascaSarjanaViewModel viewModel, int ID_Vakasi)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
        ...
        public bool DeleteVakasiHonor(HonorPascaSarjanaModel.HonorPascaSarjanaViewModel viewModel, int ID_Vakasi)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
