...
{
    public class HonorPascaSarjanaDAO
    {
        public async Task<IEnumerable<HonorPascaSarjanaModel>> ShowHonorPascaSarjanaAsync(string NPPFilter = null, string NAMAFilter = null)
        {
            var connectionString = DBkoneksi.payrollkoneksi;

            if (string.IsNullOrEmpty(NPPFilter) && string.IsNullOrEmpty(NAMAFilter))
            {
                return new List<HonorPascaSarjanaModel>();
            }
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    var query = @"SELECT DISTINCT
                                ...
                                WHERE 
                                    MST_KOMPONEN_GAJI.ID_KOMPONEN_GAJI BETWEEN 175 AND 182";
                    ...
                    return await conn.QueryAsync<HonorPascaSarjanaModel>(query, parameters);
                }
        ...
        public int InsertHonorPascaSarjana(HonorPascaSarjanaModel model)
        {
        ...
        public int UpdateHonorPascaSarjana(List<HonorPascaSarjanaModel> model)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
        ...
        public int DeleteHonorPascaSarjana(List<HonorPascaSarjanaModel> model)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
        ...
        public async Task<IEnumerable<HonorPascaSarjanaModel>> GetKomponenGaji()
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    var query = @"SELECT 
                                ...
                                WHERE ID_KOMPONEN_GAJI BETWEEN 175 AND 182;";

                    var data = await conn.QueryAsync<HonorPascaSarjanaModel>(query);
                ...
                catch (Exception)
                {
                    ...
                    return Enumerable.Empty<HonorPascaSarjanaModel>();
                }
            }
        }

        // Support untuk Dropdown Insert ID_BULAN_GAJI
        public async Task<IEnumerable<HonorPascaSarjanaModel>> GetBulanGaji(int tahun = 0)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    ...
                    var result = data.Select(id_bulan => new HonorPascaSarjanaModel
                    {
