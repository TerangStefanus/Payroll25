...
namespace Payroll25.DAO
{
    ...
    {
        public async Task<IEnumerable<BebanMengajarModel>> ShowBebanMengajarAsync(string prodi = null, string namaMK = null)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    ...
                    query += @"MKARY.[NPP],
                       ...
                       JOIN [PAYROLL].[dbo].[tbl_kelas] AS KLS ON MKARY.[NPP] = KLS.[NPP_DOSEN1]
                       JOIN [PAYROLL].[dbo].[tbl_matakuliah] AS MK ON KLS.[KODE_MK] = MK.[KODE_MK]
                       JOIN [PAYROLL].[payroll].[TBL_BEBAN_MENGAJAR] AS BM ON MKARY.[NPP] = BM.[NPP]
                       JOIN [PAYROLL].[dbo].[tbl_semester_akademik] AS SA ON KLS.[ID_TAHUN_AKADEMIK] = SA.[ID_TAHUN_AKADEMIK]
                       JOIN [PAYROLL].[dbo].[ref_prodi] AS PRD ON PRD.[ID_PRODI] = KLS.[ID_PRODI_BUAT]
                       JOIN [PAYROLL].[dbo].[ref_fakultas] AS FKL ON FKL.[ID_FAKULTAS] = PRD.[ID_FAKULTAS]";
