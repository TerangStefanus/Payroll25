...
namespace Payroll25.DAO
{
    ...
    {
        public async Task<IEnumerable<HonorUjianModel>> ShowHonorUjianAsync(string NPPFilter = null, string NAMAFilter = null)
        {
            var connectionString = DBkoneksi.payrollkoneksi;

            if (string.IsNullOrEmpty(NPPFilter) && string.IsNullOrEmpty(NAMAFilter))
            {
                return new List<HonorUjianModel>();
            }
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    var query = @"SELECT DISTINCT
                                ...
                                WHERE 
                                    MST_KOMPONEN_GAJI.ID_KOMPONEN_GAJI BETWEEN 78 AND 80";
                    ...
                    return await conn.QueryAsync<HonorUjianModel>(query, parameters);
                }
        ...
        public int InsertHonorUjian(HonorUjianModel model)
        {
        ...
        public int UpdateHonorUjian(List<HonorUjianModel> model)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
        ...
        public int DeleteHonorUjian(List<HonorUjianModel> model)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
        ...
        public async Task<IEnumerable<HonorUjianModel>> GetKomponenGaji()
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    var query = @"SELECT 
                                ...
                                WHERE ID_KOMPONEN_GAJI BETWEEN 78 AND 80;";

                    var data = await conn.QueryAsync<HonorUjianModel>(query);
                ...
                catch (Exception)
                {
                    ...
                    return Enumerable.Empty<HonorUjianModel>();
                }
            }
        }

        // Support untuk Dropdown Insert ID_BULAN_GAJI
        public async Task<IEnumerable<HonorUjianModel>> GetBulanGaji(int tahun = 0)
        {
            using (SqlConnection conn = new SqlConnection(DBkoneksi.payrollkoneksi))
            {
                try
                {
                    ...
                    var result = data.Select(id_bulan => new HonorUjianModel
                    {
